...
export default async function SaasAdminMembersPage() {
  ...
  const totalMembers = organizations.reduce((s, o) => s + o.users.length, 0);
      ...
      <div className="card" style={{ padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 2 }}>
            {totalMembers} total members across {organizations.length} organizations
          </div>
        </div>
      </div>
        ...
        <div key={org.id} className="card" style={{ overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border-color)", display: "flex", alignItems: "center", gap: 10, background: "var(--bg-page)" }}>
            ...
            <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 99,
              background: org.plan === "ENTERPRISE" ? "#fef3c7" : org.plan === "PRO" ? "#ede9fe" : "#cffafe",
              color: org.plan === "ENTERPRISE" ? "#d97706" : org.plan === "PRO" ? "#6366f1" : "#0891b2"
            }}>{org.plan}</span>
            <span style={{ fontSize: 12, color: "var(--text-muted)", marginLeft: 8 }}>{org.users.length} member{org.users.length !== 1 ? "s" : ""}</span>
          </div>

          {org.users.length === 0 ? (
            <div style={{ padding: "20px 24px", fontSize: 13, color: "var(--text-muted)" }}>No members yet.</div>
          ) : (
            <table className="data-table">
              <thead>
                <tr><th>#</th><th>Name</th><th>Email</th><th>Role</th><th>Clerk User ID</th><th>Joined</th></tr>
              </thead>
              <tbody>
                {org.users.map((u, i) => (
                  <tr key={u.id}>
                    ...
                    <td><a href={`mailto:${u.email}`} style={{ color: "#6366f1", fontSize: 13, textDecoration: "none" }}>{u.email ?? "—"}</a></td>
                    <td>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 99,
                        background: u.role === "ADMIN" ? "#ede9fe" : "#dcfce7",
                        color: u.role === "ADMIN" ? "#6366f1" : "#15803d"
                      }}>{u.role}</span>
                    </td>
